...
const ReportSubjectCard = ({ subject, time, title }: IReportSubjectCardProps) => {
    const { subjectBg } = CommonUtil.getTodoCardStyle(subject);
    ...
    return (
        <div className="w-full rounded-[14px] bg-[#FEFEFE] border border-grayscale-border shadow-[0px_2px_3px_0px_#00000012_inset] flex pr-[8px]">
            <div className={`p-[14px] rounded-tl-[14px] rounded-bl-[14px] flex items-center justify-center text-[14px] text-grayscale-dark-gray ${subjectBg}`}>{SUBJECT_NAME[subject as keyof typeof SUBJECT_NAME]}</div>
