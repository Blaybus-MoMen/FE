        ...
        <div className="h-full w-full flex flex-col min-h-0 bg-[#e1ecff]">
            ...
            <div className="overflow-auto flex-1 min-h-0 flex flex-col gap-[12px] pt-[14px] px-[13px] pb-[80px] no-scrollbar">
                {!data?.length ? (
                    ...
                    data.map((todo) => (
                        <TodoCard key={todo.todoId} todoId={todo.todoId} title={todo.title} subject={todo.subject} goalDescription={todo.goalDescription} mentorConfirmed={todo.mentorConfirmed} hasFeedback={todo.hasFeedback} studyTimeHours={todo.studyTimeHours} studyTimeMinutes={todo.studyTimeMinutes} studyTimeSeconds={todo.studyTimeSeconds} date={formattedSelectedDate} />
                    ))
                )}
            </div>
            <button
