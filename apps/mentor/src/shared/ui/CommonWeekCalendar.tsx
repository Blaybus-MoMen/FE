    ...
    return (
        <div className="w-full border-none flex flex-col gap-4 md:gap-[35px] bg-transparent rounded-[24px] px-4 py-3 md:px-6 md:py-5">
            <div className="react-calendar__navigation_mo">
                <button onClick={() => moveMonth(-1)} className={clsx("px-2", classNames?.arrow)}>
                    <img src={arrowLeftIcon} alt="이전 달" />
                </button>
                <button onClick={onOpenSheet} className={clsx("font-semibold", classNames?.header)}>
            ...
            <div className="flex items-center justify-between">
                <button
                    onClick={() => moveWeek(-1)}
                    className={clsx("text-grayscale-bg-gray text-[24px]", classNames?.arrow)}
                >
                    ...
                    {days.map((day) => {
                        ...
                        return (
                            ...
                            >
                                <span
                                    className={clsx(
                                        "text-[12px] md:text-[16px] leading-[1.4] mb-[4px] md:mb-[8px]",
                                        classNames?.weekday ?? "text-point-yellow",
                                    )}
                                >
                                ...
                                <span
                                    className={clsx(
                                        "text-[0.875rem] md:text-[1rem] leading-[1.7] flex items-center justify-center w-[30px] h-[30px] md:w-[38px] md:h-[38px]",
                                        isSelected && [
                                            "rounded-full",
                                            classNames?.selectedDate ?? "bg-point-yellow text-black",
                                        ],
                                        !isSelected && [
                                            classNames?.date ?? "text-grayscale-bg-gray",
                                        ],
